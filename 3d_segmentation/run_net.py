...
def get_xforms(mode="train", keys=("image", "label")):
    ...
    xforms = [
        LoadImaged(keys),
        AddChanneld(keys),
        Orientationd(keys, axcodes="LPS"),  # 方向变换
        Spacingd(keys, pixdim=(1.25, 1.25, 5.0), mode=("bilinear", "nearest")[: len(keys)]),  # 体素间距变换
        ScaleIntensityRanged(keys[0], a_min=-1000.0, a_max=500.0, b_min=0.0, b_max=1.0, clip=True),  # 强度变换
    ]
    if mode == "train":
        xforms.extend(
            [
                SpatialPadd(keys, spatial_size=(192, 192, -1), mode="reflect"),  # ensure at least 192x192  空间裁剪
                RandAffined(  # 放射变换
                    keys,
                    prob=0.15,
                ...
                RandCropByPosNegLabeld(keys, label_key=keys[1], spatial_size=(192, 192, 16), num_samples=3),  # 按阴性阳性比裁剪
                RandGaussianNoised(keys[0], prob=0.15, std=0.01),  # 添加高斯噪声
                RandFlipd(keys, spatial_axis=0, prob=0.5),  # 随机翻转
